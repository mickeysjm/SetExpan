...
void line_adjacency::init(line_hin *p_hin, char edge_type, int mode)
{
    ...
    for (int k = 0; k != node_u->node_size; k++) u_len[k] = sqrt(u_len[k]);
...
line_trainer_l1::line_trainer_l1()
{
    edge_tp = 0;
    phin = NULL;
    u_nb_cnt = NULL;
...
void line_trainer_l1::init(line_hin *p_hin, char edge_type)
{
    edge_tp = edge_type;
    phin = p_hin;
    line_node *node_u = phin->node_u, *node_v = phin->node_v;
...
void line_trainer_l1::train_uv(int u, int v, real lr, real margin, real *_error_vec, double randv)
{
    int n, vector_size;
    real dpos, dneg;
    line_node *node_u = phin->node_u, *node_v = phin->node_v;
    ...
    if (dneg - dpos < margin)
    {
        ...
        {
            x = node_u->vec(u, c) - node_v->vec(v, c);
            if (x > 0) x = 1;
            else x = -1;
            node_v->vec(v, c) += lr * x;
            error_vec(c) -= lr * x;
            
            x = node_u->vec(u, c) - node_v->vec(n, c);
            if (x > 0) x = 1;
            else x = -1;
            node_v->vec(n, c) -= lr * x;
            error_vec(c) += lr * x;
...
void line_trainer_l1::train_sample(real lr, real margin, real *_error_vec, double (*func_rand_num)())
{
    int u, v, index;
    ...
    train_uv(u, v, lr, margin, _error_vec, func_rand_num());
}

void line_trainer_l1::train_sample_depth(real lr, real margin, real *_error_vec, double (*func_rand_num)(), int depth, line_adjacency *p_adjacency, char pst)
{
    int u, v, index;
    std::vector<int> node_lst;
...
line_trainer_l2::line_trainer_l2()
{
    edge_tp = 0;
    phin = NULL;
    u_nb_cnt = NULL;
...
void line_trainer_l2::init(line_hin *p_hin, char edge_type)
{
    edge_tp = edge_type;
    phin = p_hin;
    line_node *node_u = phin->node_u, *node_v = phin->node_v;
...
void line_trainer_l2::train_uv(int u, int v, real lr, real margin, real *_error_vec, double randv)
{
    int n, vector_size;
    real dpos, dneg;
    line_node *node_u = phin->node_u, *node_v = phin->node_v;
...
void line_trainer_l2::train_sample(real lr, real margin, real *_error_vec, double (*func_rand_num)())
{
    int u, v, index;
...
void line_trainer_l2::train_sample_depth(real lr, real margin, real *_error_vec, double (*func_rand_num)(), int depth, line_adjacency *p_adjacency, char pst)
{
    int u, v, index;
    std::vector<int> node_lst;
...
void line_triple::init(const char *file_name, line_node *p_h, line_node *p_t, line_node *p_r)
{
    ...
    triple_size = 0;
    while (fgets(str, sizeof(str), fi)) triple_size++;
    fclose(fi);
    ...
    {
        fscanf(fi, "%s %s %s", sh, st, sr);
        
        if (k % 10000 == 0)
        {
            printf("Reading edges: %.3lf%%%c", k / (double)(triple_size + 1) * 100, 13);
            fflush(stdout);
        }
...
void line_triple::train_sample(real lr, int dis_type, int h, int t, int r, int nh, int nt, int nr)
{
    ...
    for (int c = 0; c != vector_size; c++)
    {
        if (dis_type == 1)
        {
            x = 2 * (node_t->vec(t, c) - node_h->vec(h, c) - node_r->vec(r, c));
            if (x > 0) x = 1;
            else x = -1;
        }
        else if (dis_type == 2)
        ...
        if (dis_type == 1)
        {
            x = 2 * (node_t->vec(nt, c) - node_h->vec(nh, c) - node_r->vec(nr, c));
            if (x > 0) x = 1;
            else x = -1;
        }
        else if (dis_type == 2)
...
void line_triple::train_batch(real lr, real margin, int dis_type, double (*func_rand_num)())
{
    ...
    if (coin < 0.5)
    {
        ...
        {
            sp = (node_h->vec.row(h) + node_r->vec.row(r) - node_t->vec.row(t)).array().pow(2).sum();
            sn = (node_h->vec.row(neg) + node_r->vec.row(r) - node_t->vec.row(t)).array().pow(2).sum();
        }
        
        if (sn - sp < margin)
        {
            train_sample(lr, dis_type, h, t, r, neg, t, r);
        }
    }
    else
    {
        ...
        {
            sp = (node_h->vec.row(h) + node_r->vec.row(r) - node_t->vec.row(t)).array().pow(2).sum();
            sn = (node_h->vec.row(h) + node_r->vec.row(r) - node_t->vec.row(neg)).array().pow(2).sum();
        }
        
        if (sn - sp < margin)
        {
            train_sample(lr, dis_type, h, t, r, h, neg, r);
        }
    }
}
